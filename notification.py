...
def enviar_whatsapp(Nome, Telefone):
    ...
    message = client.messages.create(
        content_sid='HX5eeb30c564c0da14b8688f8decf28394',
...
'''
            ...
            # Enviar email
            try:
                enviar_email(aluno['Email'], 'Sua Mensalidade da Empresa Prelúdio Vence Hoje.', corpo_email, 'onlinepreludio@gmail.com', 'rudo ydca fnyn nzju')
...
def chamar_funcoes():
    ...
    global dados
    # Chamar a função enviar_notificacoes_intervalo() passando os dados dos alunos
    enviar_notificacoes_intervalo(dados)

def enviar_notificacoes_intervalo(dados):
    intervalo_data_button.state(["disabled"])
    data_inicial = pd.to_datetime(intervalo_data_inicial_button.entry.get())
    data_final = pd.to_datetime(intervalo_data_final_button.entry.get())

    for index, aluno in dados.iterrows():
        data_vencimento = pd.to_datetime(aluno['Data de Vencimento'])
...
# Criando o botão e associando-o à função que chama as duas funções
intervalo_data_button = create_button_with_icon("Enviar Notificações por Data", chamar_funcoes, "success.large", icon_path + "exibir_vencendo_icon.png")
intervalo_data_button.pack(pady=20)

# Botao para escolher o intevalo de datas dos alunos
intervalo_data_inicial_button = ttk.DateEntry(root, bootstyle="success", firstweekday=0)
intervalo_data_inicial_button.pack(pady=20)

# Botao para escolher o intevalo de datas dos alunos (final)
intervalo_data_final_button = ttk.DateEntry(root, bootstyle="success", firstweekday=0)
intervalo_data_final_button.pack(pady=20)
