    ...
    for index, aluno in dados.iterrows():
        ...
        if aluno['Data de Vencimento'].day == data_atual.day:
            ...
            corpo_email = f'''
                ...
                <p>Vimos por meio deste comunicar que há um débito pendente em sua conta referente à Empresa..</p>
                <p>Solicitamos, por gentileza, que providencie o pagamento o mais breve possível por meio do PIX para a seguinte chave: x@gmail.com. Pedimos que encaminhe o comprovante de pagamento para este mesmo endereço de e-mail ou através do número x.</p>
                <p>Agradecemos antecipadamente pela sua atenção e colaboração neste assunto.</p>
                <p>Atenciosamente,</p>
                <p>Empresa. 🎹🎵</p>
                <p><img src="x.png" alt="Logo"></p>
'''


            # Enviar email
            try:
                enviar_email(aluno['Email'], 'Sua Mensalidade da Empresa Vence Hoje.', corpo_email, 'x@gmail.com', 'password')
...
def exibir_alunos_vencendo_hoje():
    ...
    if not alunos_vencendo_hoje.empty:
        ...
        for index, row in alunos_vencendo_hoje.iterrows():
            ...
            tree.insert('', 'end', values=(row['Nome'], row['Email'], row['Telefone'], data_vencimento_formatada, status_pagamento, row['Mensalidades Atrasadas']))
...
def recarregar_janela_alunos(tree, dados, file_path):
    ...
    for index, row in dados.iterrows():
        ...
        values_list = [row['Nome'], row['Email'], row['Telefone'], data_vencimento_formatada, status_pagamento, str(row['Mensalidades Atrasadas'])]
...
def exibir_alunos():
    ...
    for index, row in dados.iterrows():
        ...
        item_id = tree.insert('', 'end', values=(row['Nome'], row['Email'], row['Telefone'], data_vencimento_formatada, status_pagamento, row['Mensalidades Atrasadas']), tags=(f'I{index}', 'checkbox'))
        tree.tag_configure(f'I{index}', background='#333333', foreground='white')  # Ajuste opcional para a cor de fundo
        tree.insert(item_id, 'end', values='', tags=(f'I{index}', 'checkbox'))
        ...
        if tags and tags[0].startswith('I'):  # Verificar se a tag começa com 'I' (indicando um índice)
            ...
            if index_str.isdigit():
                ...
                if 0 <= index < len(dados):
                    ...
                    tree.item(item_selecionado[0], values=(dados.iloc[index]['Nome'], dados.iloc[index]['Email'], dados.iloc[index]['Telefone'], dados.iloc[index]['Data de Vencimento'], "Pagou" if dados.iloc[index]['Pagou Mensalidade'] else "Não Pagou", dados.iloc[index]['Mensalidades Atrasadas']))
...
def atualizar_exibicao(tree, index):
    ...
    tree.item(item_selecionado[0], values=(row['Nome'], row['Email'], row['Telefone'], row['Data de Vencimento'], status_pagamento, row['Mensalidades Atrasadas']))
...
def voltar_para_false_todos(tree, dados, file_path):
    ...
    if resposta == 'yes':
        ...
        # Preencher a tabela com os dados atualizados
        for index, row in dados.iterrows():
            values_list = list(row[['Nome', 'Email', 'Telefone', 'Data de Vencimento', 'Pagou Mensalidade', 'Mensalidades Atrasadas']].astype(str))
            item_id = tree.insert('', 'end', values=values_list, tags=(f'I{index}', 'checkbox'))
            # Configurar a cor de fundo como #333333
            tree.tag_configure(f'I{index}', background='#333333', foreground='white')  # Ajuste opcional para a cor de texto
            tree.insert(item_id, 'end', values='', tags=(f'I{index}', 'checkbox'))
        ...
        if tags and tags[0].startswith('I'):  # Verificar se a tag começa com 'I' (indicando um índice)
            ...
            if index_str.isdigit():
                ...
                if 0 <= index < len(dados):
                    ...
                    tree.item(item_selecionado[0], values=(dados.iloc[index]['Nome'], dados.iloc[index]['Email'], dados.iloc[index]['Telefone'], dados.iloc[index]['Data de Vencimento'], status_pagamento, dados.iloc[index]['Mensalidades Atrasadas']))
